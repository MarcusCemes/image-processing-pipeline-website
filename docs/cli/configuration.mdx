...
default searching behavior and instead exclusively attempt to use the provided config path.
...
current working directory and parent directories for a recognized configuration file. Any file
...
- **JSON** is a widely recognized universal format, similar to JS
...
is correct, an incorrect configuration could cause undetermined execution behavior.
...
schema][config-schema]. This strict behavior ensures that there are no misspelt property names or
deprecated keys.
...
To get a more standardized and always up-to-date API reference, check out the configuration [JSON
...
| errorFile   | string, boolean _or_ function | Disable `false`, change path `string` or use a callback function |
...
<!-- prettier-ignore-start -->
:::note
`concurrency` is not actually required as it is populated by the initialization script by default.
:::
<!-- prettier-ignore-end -->

<!-- prettier-ignore-start -->
:::danger
When the `clean` option is enabled, the path specified as the output folder will be unrecoverable
