...
default searching behaviour and instead exclusivly attempt to use the provided config path.
...
IPP eventually converts configuations to JS, but understands several base formats. Each have slight
...
Calling the CLI's `startCLI()` function programatically **bypasses any configuration searching or
validation**. Instead, the configuration is passed as a function parameter.

Due to the dynamic nature of JavaScript, it is your responsiblity to ensure that the configuration
...
features offered by your IDE by specifying a root `$schema` property that points to [this permanant
...
<!-- prettier-ignore-start -->
:::danger
When the `clean` option is enabled, the path specified as the output folder will be unrecoverably
...
| key     | type                                          | description                                     |
| :------ | :-------------------------------------------- | :---------------------------------------------- |
| pipe    | string _or_ object _or_ function              | name or object of pipe _(required)_             |
| options | object                                        | pipe specific options                           |
| test    | boolean _or_ string _or_ RegExp _or_ function | if truthy, executes the pipe for the given file |
| save    | string _or_ `true`                            | save the file to output                         |
| then    | object                                        | execute pipe(s) after this one                  |
...
This property tests each file paths and the file will be processed if it returns truthy. It can be:

- keep **undefined** (default) or use **true** to skip testing and always proceed
- a **string** to check for explicit file name
- a **RegExp** to do a regex test against the file name
- a **function** callback (advanced usage) to do custom testing

Also a combination of multiple **RegExp** and **function** is possible.

#### Examples

JavaScript / Typescript configuration is recommended to allow more advanced behavior.

Use test to only process one file:

<Tabs
  groupId="example_pipe_test_str"
  defaultValue="yaml"
  values={[
    {label: 'YAML', value: 'yaml'},
    {label: 'JSON', value: 'json'},
...
const pipe = {
  pipe: "passthrough",
  test: "relative/dir/file.png", // only process file.png
};
```
...
Use test with callback to process only png files:

<Tabs
  groupId="example_pipe_test_callback"
...
const pipe = {
  pipe: "passthrough",
  test: (filepath: string, _metadata: Metadata) => filepath.endsWith(".png"), // process png only
};
```
...
Use test with RegExp to process only png files:

<Tabs
  groupId="example_pipe_test_regexp"
  defaultValue="yaml"
  values={[
    {label: 'YAML', value: 'yaml'},
    {label: 'JSON', value: 'json'},
...
Use test with multiple rules (all must be truthy):

<Tabs
  groupId="example_pipe_test_regexp_callback"
  defaultValue="typescript"
  values={[
    {label: 'TypeScript', value: 'typescript'},
  ]
...
const pipe = {
  pipe: "passthrough",
  // only process png files with a width >= 128 px
  test: [/\.png$/, (_filepath: string, metadata: Metadata) => metadata.source.width >= 128],
};
```
